            ...
            <div class="card card-outline card-primary">
                <div class="card-header">
                    ...
                    <div class="btn-group btn-group-toggle" data-toggle="buttons">
                        ...
                        <a href="{{ url('admin/sellers/showInactiveSellers') }}" class="btn bg-secondary">
                    ...
                    <table id="example1" class="table table-striped table-bordered table-hover table-sm text-center">
                        ...
                        <tbody>
                            @foreach ($users as $user)
                                @if ($user['role'] !== 'admin' && $user['role'] !== 'user' && $user['status'] !== 'inactivo')
                                    <tr>
                                        <td>{{ $user->id }}</td>
                                        <td>{{ $user->name }}</td>
                                        <td>{{ $user->email }}</td>
                                        <td>
                                            <div class="btn-group" role="group" aria-label="Basic example">
                                                <a href="{{ url('admin/sellers/' . $user->id.'/show') }}" type="button" class="btn btn-info btn-sm" title="Ver"><i class="bi bi-eye"></i></a>
                                                <a href="{{ url('admin/sellers/' . $user->id . '/edit') }}" type="button" class="btn btn-success btn-sm" title="Editar"><i class="bi bi-pencil"></i></a>
                                                <a href="{{ url('admin/sellers/' . $user->id . '/confirmDeleteSeller') }}" type="button" class="btn btn-danger btn-sm" title="Eliminar"><i class="bi bi-trash"></i></a>
                                            </div>
                                        </td>
                                    </tr>
                                @endif
